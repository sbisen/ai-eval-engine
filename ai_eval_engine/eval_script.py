"""Step 3 (artifact) — generate a standalone, editable eval runner.

The pipeline emits a small ``eval_<project>.py`` next to the golden set. It is a
readable harness a team can commit and adapt: point ``predict()`` at your real
agent, and it scores against the golden set, updates the runbook, and refreshes
the dashboard. The generated file depends only on the installed ``ai_eval_engine``
package, so it stays short and reviewable.
"""
...
def generate_eval_script(project: str) -> str:
    ...
    return _TEMPLATE.format(project=project, filename=f"eval_{project}.py")
